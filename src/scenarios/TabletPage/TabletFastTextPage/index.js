...
import {getLocationState, isSafari, onDownload, setLocationState} from '../../../utils/util';
  ...
  const proceedRolePlay = async () => {
    ...
    if (allTask && allTask instanceof Array) {
      ...
      if (index != -1 && index < allTask.length - 1) {
        ...
        setLocationState(
            {isShowStory: false, isShowKururinpaImg: true, commitId: store.getState().conduct_all_section_id},
            ...
            component={
              isVertical ? <>
                    ...
                    <div className={styles.fast_text_main_page}>
                      ...
                      <div className={styles.fast_text_main_bottom}>
                        ...
                        <div className='d-flex mr-2 mt-2'>
                          ...
                          >
                            {vIsplayed && cameraOnUse ? <WebCam setCamera={v => setCameraOnUse(v)}/> :
                                <img src={humanAvatar}/>}
                          </div>
                          <textarea
                          ...
                          <div>
                            <div className={styles.timer_area}>
                              {delChatMsg === 'stt' && sttOpened && !inFocus ?
                                  <IWaveformMobile value={volume} isTablet={true}/>
                                  :
                                  <IWaveformMobileGrey isTablet={true}/>
                              }
                            </div>
                    ...
                    <div className={styles.fast_text_controller_btns}>
                      <div className={styles.fast_text_bottom_left_btns}>
                        {/** やり直す ⬇ */}
                        <button
                            className={vIsplayed ? styles.fast_text_bottom_left_btn_started : styles.fast_text_bottom_left_btn}
                            onClick={() => {
                              if (!ableToNext()) {
                      ...
                      <div className={styles.fast_text_bottom_middle_btns}>
                        {/* 反映 */}
                        <button
                  ...
                  <>
                    ...
                    {
                      vTask?.chat?.section.persona.themeName ?
                          <div className={styles.fast_text_name_context_landscape}>
                            <label
                                title={`${vTask?.chat?.section.persona.themeName} ${vTask?.chat?.section.persona.scenarioName}`}>{`${vTask?.chat?.section.persona.themeName} ${vTask?.chat?.section.persona.scenarioName}`}</label>
                            <label
                                title={`${vTask?.chat?.section.persona.persona}`}>{`${vTask?.chat?.section.persona.persona}`}</label>
                    ...
                    <div className={styles.fast_text_main_page_landscape}>
                      <Row className={styles.fast_text_main_show_landscape}>
                        <Col sm={5} className='d-flex flex-column justify-content-between'>
                          ...
                          <div className={styles.fast_text_toggle_btns_landscape}>
                            <div>
                              <ShowBut
                                  onChange={handleChange}
                                  checkState={overallScoring}
                        ...
                        <Col>
                          <div className={styles.fast_text_page_avatar_landscape}>
                      ...
                      <Row className={styles.fast_text_main_bottom_landscape}>
                    ...
                    <Row className={styles.fast_text_controller_btns_landscape}>
                      <Col sm={5}>
                        {/** やり直す ⬇ */}
                        <button
                            className={vIsplayed ? styles.fast_text_bottom_left_btn_started_landscape : styles.fast_text_bottom_left_btn_landscape}
                            onClick={() => {
                              if (!ableToNext()) {
                                setIsOpenStartOverDialog(true);
                              }
                      ...
                      <Col sm={4}>
                        ...
                        <button
                            ...
                            className={`${styles.role_play_handle_btn_landscape} ${vIsplayed ? styles.role_play_reflect_btn_active : styles.role_play_handle_btn_not_start}`}
                            autoFocus={vSetSendFocus}
                            onClick={dealIconHide}
                            id={autoId()}
                        >
                        ...
                        {/* 重新输入 */}
                        <button
                            className={`${styles.role_play_handle_btn_landscape} ${vIsplayed ? styles.role_play_respeak_btn_active : styles.role_play_handle_btn_not_start}`}
                            onClick={vIsplayed ? delChatMsg !== 'input' ? dealCancel : dealCancelAndBackStt : null}
                            id="cancel_icon"
                            autoFocus={vSetDelFocus}
                            name="cancel_icon"
