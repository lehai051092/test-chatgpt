...
import PdfIcon from "../../../property/images/link.png"
                ...
                return <div className={classes.foldableArea} key={index}>
                            <div className={classes.purpose_sec}>
                                <span id="rate_of_risk_header" name="rate_of_risk_header">
                                    ...
                                    {
                                        vShowToggleBtn && <span className={`${classes.toggleBtn} ${selectTask[index].showStatus ? classes.button_arrow_up : classes.button_arrow_down}`} onClick={() => toggleFoldingArea(index, select_task, selectTask[index].showStatus)}>                                                   
                                        </span>
                                    }
                                </span>
                            </div>
                                            ...
                                            pdfList.map((tableRow, materialIndex) => { 
                                                if(tableRow && tableRow.task_ID == select_task.taskID){
                                                        ...
                                                        return <Row className={classes.pdf_row} key={materialIndex}>
                                                            {
                                                                tableRow.referenceMaterials.map((material, materialIndex1)=> {  
                                                                    return  <div className={classes.div_pdf} key={materialIndex1}>  
                                                                                <img src={PdfIcon} className={classes.img_left}/>
                                                                                <span className={`mr-2 ${classes.cursor_pointer}`}> <a href="javascript:void(0);" className={classes.linkFlie} onClick={() => downloadFile(material,material.referenceName)} key={materialIndex}>{material.referenceName}</a> </span>
                                                                            </div>
                                                                })
                                                            } 
                                                        </Row>
                                    ...
                                    <div className={`mt-1 pb-2 cmn-scroll-bar ${classes.scroll_container}`}>
                                            {
                                                selectTask[index].showStatus && vAiScore.get(select_task.taskID).map((item, index) => {
                                                    ...
                                                    return  <Link to={{pathname:`/ai-score/${select_task.taskID}/${item.section.persona.id}` }} 
                                                                key={index}
                                                                onClick={()=>{setLocationState({ item }, `ai-score/${select_task.taskID}/${item.section.persona.id}`)}}>
