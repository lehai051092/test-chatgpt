        ...
        <div className={`${classes.scroll_bar} ${item.commitId != ""?'':classes.scroll_bar_hide_before}`} id={autoId()} name={autoId()}>
                <Row className={classes.time_style}>
                    <Col className={classes.col_style}>
                        <span id={`${id}_date_text`} name={`${id}_date_text`}>{date(item)}{getStartTime(item)}</span>
                        <span id={`${id}_time_diff`} name={`${id}_time_diff`} > - {getFinishedDate(item)}{getFinishedTime(item)}</span>
                    </Col>
                    <Col className={classes.col_style}>
                        {
                            item.commitId != "" ?
