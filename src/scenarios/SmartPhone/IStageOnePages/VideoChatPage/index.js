...
import {
  deleteScoring,
  finishScoring,
  getChatIdListByCommitIdForMultipleSection, getChatMatchWords, getCheckSavedChat,
...
import timer_sp_landscape_icon from '../../../../property/icons/timer_for_sp_landscape.svg';
...
import ShowButTalkScript from "../../../TabletPage/IStageOnePages/VideoChatPage/ShowButTalkScript";
...
const VideoChatPage = ({rolePlayingSavedDuringProcess}) => {
  ...
  const hasRolePlayingData = rolePlayingSavedDuringProcess
  ...
  const GetTask = async () => {
    ...
    let response;
    const savedChat = rolePlayingSavedDuringProcess
  ...
  const dealSingleScore = async () => {
    ...
    setLocationState({
      ...
      task: store.getState()?.select_task[0]
...
//
    ...
    if (allTask && allTask instanceof Array) {
      ...
      if (state && state.pname == 'multiple-scenarios' && location.pathname.includes('multiple-scenarios') && store.getState().select_task.length > 0 && index < allTask.length - 1) {
        ...
        return <button onClick={ableToNext() ? () => {
                       ...
                       className={ableToNext() ? classes.disable : classes.multiple_button}>次へ</button>
      } else if (location.pathname.includes('multiple-scenarios')) {
        // if multiple last time
        ...
        return <button onClick={ableToNext() ? () => {
        } : () => setIsOpenMultiConfirmDialog(true)}
                       className={ableToNext() ? classes.disable : classes.single_button}>採点する</button>
      } else {
        // if single
        // return <img width={'100%'} className={classes.video_chat_page_delete_pad_flex_once_submit} src={mobileFreeStorySubmit} onClick={ableToNext() ? () => { } : () => setIsOpenConfirmDialog(true)} />
        return <button onClick={ableToNext() ? () => {
  ...
  const startOrRestartOrSaveButton = useMemo(() => {
    if (vIsplayed) {
      return (
          <div
              className={`${classes.start_role_play_btn_box}`}
          >
            <FreeStoryStopBtn
                onClick={async () => {
      ...
      return (
          ...
          >
            <button
                className={`${classes['start-role-play-btn']}`}
                disabled={vIsplayed}
                onClick={async () => {
                  store.dispatch(updateTempRolePlaying(vTask?.chat))
                  await startRolePlay()
  ...
  return (
      <ICoreFrame
          ...
          component={vIsVertical ? <>
            <div className={classes.main_content_inr_body_sp}>
              <div className={classes.main_content_inr_child_box_sp}>
                <div className={classes.video_chat_name_context}>
                  {/* <button title={t('general.return')} className="video-chat-btn-click-back" onClick={dealReturn}>{t('general.return')}</button> */}
                  {
                ...
                <div className={classes.role_play_main_area_sp}>
                  <div classsName={classes.video_chat_list_title_sp}>
                    <p className={classes.video_chat_list_first_title} id="ongoing_section"
                    ...
                    <span className="video-chat-top-buttons">
                                    <div title={vTask && `${vTask.chat.section.name}`}
                                         className={classes.video_chat_btn_tips} id="long_term_care_insurance"
                                         name="long_term_care_insurance">
                                        {
                                            vTask &&
                                            (vTask?.chat?.section?.name).split('\n').map((v, k) => {
                                              return (<div>
                                                <p style={{textTransform: 'none'}} key={k}>{v}</p>
                                              </div>)
                                            })
                                        }
                                    </div>
                  ...
                  <div className={classes.flex_mobile_column_reverse}>
                    <Col className={`${classes.chat_avatar_area} mb-sm-`}>
                      ...
                      <div className={classes.video_chat_keyword_list_main}>
                        <div className={classes.video_chat_keyword_list}>
                          <Row>
                            ...
                            <Col lg='4' style={{marginBottom: vTalkScript ? '25px' : '10px'}}>
                              {
                      ...
                      <div className={`mb-20 ${classes.video_chat_input_area}`}>
                        <div className={classes.video_chat_input_area_box}>
                          <div className={classes.chat_people_box}>
                            <div>
                              {name &&
                                  <div className={classes.chat_name_box_sp}>
                                    {name}
                                  </div>
                              }
                              {chatPerson &&
                          ...
                          <div className={classes.vitual_chat_avatar}>
                            {
                              delChatMsg === 'stt' && sttOpened && !inFocus
                                  ?
                                  <IWaveformMobile value={volume} isVertical={true} isTablet={false}/> :
                                  <IWaveformMobileGray isVertical={true} isTablet={false}/>
                            }
                          </div>
                          ...
                          {inFocus ? (<span>テキスト編集中</span>) : null}
                        </div>
                        <div className={classes.video_chat_input_button}>
                                            <textarea
                                                ...
                                                className={`${classes.video_chat_input_sp}`}
                ...
                <div className={classes.buttons}>
                  <button className={ableToNext() ? classes.disableBtn : ''} onClick={ableToNext() ? () => {
                  } : () => {
                    setIsOpenStartOverDialog(true)
                  }}>最初からやり直す
                  </button>
                </div>
                <div style={{height: 73, width: '100%'}}></div>
              </div>
              <div className={classes.video_chat_footer_buttons_sp}>
                <div className={classes.video_chat_footer_buttons_sp_left}>
                  <button className={ableToNext() ? classes.video_chat_footer_buttons_sp_left_button_disable : ''}
                ...
                <div className={classes.video_chat_footer_buttons_sp_right}>
                  {
                    renderOperateBtn()
                  }
                </div>
            ...
            <div className={classes.role_play_main_area_landscape}>
              <div style={{paddingRight: 9}}>
                        <span className={classes.subtitle_name}>
                            実施中のセクション
                        </span>
                <div className={classes.subtitle_name_context}>
                  {
                      vTask &&
                      (vTask?.chat?.section?.name).split('\n').map((v, k) => {
                        return (<div>
                ...
                {confirmDialogReRolePlayElement}
                <div className={classes.video_chat_keyword_list_main_pad}>
                  {startOrRestartOrSaveButton}
                  <div className={classes.video_chat_keyword_list}
                       style={{marginRight: `${vTalkScript || overallScoring ? '5px' : '0'}`}}>
                    <Row>
                      <Col lg='4'>
                        <ShowButTalkScript
                            showFlg={true}
                      ...
                      <Col lg='8' className="text-right responsive_size">
                        <ShowBut onChange={handleChange} checkState={overallScoring} buttonlabel="プロセス/キーワードを表示する"/>
                      </Col>
                      <Col lg='4' style={{marginBottom: overallScoring ? '0px' : '10px'}}>
                        {
                          overallScoring ? <KeywordList vProcesses={vProcesses} setProcesses={setProcesses}
                ...
                <div className={classes.control_button_landscape_pad}>

                  <div className={classes.video_chat_footer_buttons}>
                    {/* <div className={classes.video_chat_footer_buttons_sp_left_landscape}>
                                    <button className={ableToNext() ? classes.video_chat_footer_buttons_sp_left_button_disable_landscape : ''} onClick={reflectLoading || vChatMsg.length === 0 ? () => { } : dealIconHide} id={autoId()}>反映</button>
                                    <button className={ableToNext() || delChatMsg ==='input' ? classes.video_chat_footer_buttons_sp_left_button_disable_landscape : ''} onClick={ableToNext() ? () => { } : delChatMsg ==='input' ? () => {  } : dealCancel}>言い直す</button>
                                </div> */}
                    <div className={classes.video_chat_footer_buttons_sp_left_landscape}>
                      <button
                          className={ableToNext() ? classes.video_chat_footer_buttons_sp_left_button_disable_landscape : ''}
                          onClick={reflectLoading || vChatMsg.length === 0 ? () => {
                          } : dealIconHide} id={autoId()}>反映
                      </button>
                      <button
                    ...
                    <div className={classes.video_chat_footer_buttons_sp_right_landscape}>
                      {
              ...
              <div className={classes.pdf_landscape}>
                <SliderImages/>
                <div>
                  <div className={`mb-20 ${classes.video_chat_input_area_pad}`}>
                    <div className={classes.video_chat_input_area_box_pad}>
                      <div className={classes.vitual_chat_avatar_pad}>
                        <div>
                          {name &&
                          ...
                          {chatPerson &&
                              <img src={chatPerson} alt="Chat People" className={classes.main_chat_people_pad}
                                   id="chat_person" name="chat_person" key={Math.random() * 100}/>
                          }
                        </div>
                      </div>
                    </div>
                    <div className={classes.video_chat_input_button_pad}>
                      {/** ios!! **/}
                      <textarea
                          onFocus={() => {
                            handleTextareaInputFocus()
                          ...
                          type="text"
                          value={vChatMsg}
                          onChange={(e) => onChatMsgType(e)} id="reply" name="reply" autoComplete="off"
                      />
                    </div>
                    <div className={classes.video_chat_input_area_box}>
                      <div className={classes.video_chat_input_area_right}>
                        <div className={classes.video_chat_input_area_wave}>
                          {
                            delChatMsg === 'stt' && sttOpened && !inFocus
                                ?
                                <IWaveformMobile value={volume} isVertical={false} isTablet={false}/>
                                :
                                <IWaveformMobileGray isVertical={false} isTablet={false}/>
                          }
                        </div>
                        <div className={classes.timer_pad}>
                          <img src={timer_sp_landscape_icon}/>
                          <span>{timer}</span>
                        </div>
                        {inFocus ? (<div className={classes.editing_area}><span className={classes.is_editing}>テキスト編集中</span></div>) : null}
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className={classes.video_chat_footer_buttons_pad}>
              <button className={ableToNext() ? classes.disableBtn : ''} onClick={ableToNext() ? () => {
              } : () => {
                setIsOpenStartOverDialog(true)
              }}>最初からやり直す
