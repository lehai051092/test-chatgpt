            ...
            component={<>
                <div className={`${classes.rateOfRiskPage} ${classes.position_rela}`}>
                    ...
                    <div className={classes.bottom_butt}>
                        <Link className={classes.a_style} to={{ pathname: `/fast-text/${vMultiScenraios[0]?.taskID}/${lessonId}`}}
                                onClick={()=>{setLocationState({ freeModel:true , senario:vMultiScenraios[0] }, `fast-text/${vMultiScenraios[0]?.taskID}/${lessonId}`)}}>
                            {/* <button className={`${classes.bottom_btn_submit} `} id="video_chat_page_link" name="video_chat_page_link" onClick={cacheScenarios}>
                                <span><a>{t('rateOfRisk.proceed_to_the_next')}</a><img src={ButImg}></img></span>
                            </button> */}
                            <div align={'center'}>
