...
import {addDepartment, departmentImportHistoryCSVExport, updateDepartment} from "../../../../request/backendApi/api";
...
function DepartmentEditDialog(props) {
  ...
  const handleOK = async () => {
    ...
    if (item && item.id) {
      ...
      updateDepartment(item.id, data).then((res) => {
        if (res.data) {
      ...
      <div className={styles.body}>
        ...
        <div className={styles.content}>
          <div className={styles.gridTitle}>
            <span className={styles.infoTitle}>部</span>
            <input type='text' defaultValue={departmentName} value={departmentName} className={styles.input}
                   onChange={(e) => {
                     setDepartmentName(e.target.value)
                   }} maxLength={200}/>
            <span className={styles.infoTitle}>課</span>
            <input type='text' defaultValue={sectionName} value={sectionName} className={styles.input}
                   onChange={(e) => {
                     setSectionName(e.target.value)
                   }} maxLength={200}/>
            <span className={styles.infoTitle}>略語</span>
            <input type='text' defaultValue={nameAbbr} value={nameAbbr} className={styles.input}
                   onChange={(e) => {
                     setNameAbbr(e.target.value)
                   }} maxLength={200}/>
            <span>
              <span>必須</span>
              <span className={styles.infoTitle}>コード</span>
            </span>
            <input type='text' defaultValue={code} value={code} className={styles.input}
                   onChange={(e) => {
                     if (/^[0-9]*$/.test(e.target.value)) {
                       setCode(e.target.value);
                     }
                   }} maxLength={200}/>
            <span className={styles.infoTitle}>英文名</span>
            <input type='text' defaultValue={nameEn} value={nameEn} className={styles.input}
                   onChange={(e) => {
