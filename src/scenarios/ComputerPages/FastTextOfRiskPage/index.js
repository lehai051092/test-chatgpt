...
const FastTextOfRiskPage = ({selectTask, select_task}) => {
        ...
        <ICoreFrame
            ...
            component={
                <div className={`${classes.rateOfRiskPage} ${classes.position_rela}`}>
                    <Row className={ browserRedirect() === 3 ? classes.rateOfRiskPageTitle_tablet : classes.rateOfRiskPageTitle}>
                        <Col lg="6" className={browserRedirect() === 3 ? classes.first_rwo_tablet : classes.first_row}>
                            {
                                browserRedirect() === 1?
                                <>
                                    <Link 
                                    ...
                                    >
                                        <BackButton idName="rate_of_risk_back_button" title={t('scenario.return')} className={`mr-3 ${classes.btn_adjust} ${classes.back_btn}`} id={autoId()} onClick={backOnlick}/>
                                    </Link>
                        ...
                        <Col className="text-right" lg="6" style={browserRedirect() === 3 ? {paddingRight:0} : null}>        
                            <Link 
                                to={{ pathname: `/fast-text/${vMultiScenraios[0]?.taskID}/${lessonId}`}} 
                                onClick={()=>{setLocationState(null, `fast-text/${vMultiScenraios[0]?.taskID}/${lessonId}`)}}
                            >
                                { browserRedirect() === 1 ? 
                                    <img src={RolePlayPC} width={350} className={classes.bottom_btn_submit_pc} onClick={cacheScenarios}></img>
                                    :
                                    <img src={RolePlay} width={350} className={classes.bottom_btn_submit_tablet} onClick={cacheScenarios}></img>
                                }
                            </Link>        
                        </Col>
                    </Row>
...
export default connect(stateToProps, dispatchToProps)(FastTextOfRiskPage)