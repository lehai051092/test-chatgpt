...
function KeywordList(props) {
  ...
  const isKeywordMatched = useCallback((item) => {
  ...
  return (
      <div className="video-chat-keyword-main">
        {
          vProcesses.map((item, index) => {
            return (
                <>
                  <div id={`checkbox_div_${index + 1}`} name={`checkbox_div_${index + 1}`}
                       className="video-chat-checkbox-row" key={index}>
                    <div className="d-flex justify-content-between video-chat-process-content">
                      <FormControlLabel
                          style={styles.root}
                          label={
                            <span style={styles.label} id={`check_process_${index + 1}`}
                                  ...
                                  name={`check_process_${index + 1}`}>{item.name}</span>
                          }
                          control={
                            <span></span>
                          }/>
                    ...
                    {
                      isKeywordMatched(item) ?
                          <span className={`mark-on-title-check`}/> :
                          <span className={ 'mark-on-title'}/>
                    }
                    {
                      (item.toggle) ? (
                          <div className="ml-3">
