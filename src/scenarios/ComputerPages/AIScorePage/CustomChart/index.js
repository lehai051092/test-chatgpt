...
const CustomChart = ({ aiScore, f_scoreClickHandle, isMultiple, is_flag, state, scrollAble = false, scoreData = null, chatID }) => {
  ...
  useEffect(() => {
    ...
    sorting.map((item, index) => {
      chartArray.push({
        id: index,
        month: dateFormat(item.finished),
  ...
  useEffect(() => {
    if((state != undefined) || scrollAble ||scoreData != null )
    {
      ...
      if(scrollItem > -1)
      {
        ...
        if(scrollAble && scoreData != null)
        {
          document.getElementById(chatID).scrollLeft += document.getElementById(chatID+'_'+scrollItem).getBoundingClientRect().left;
        }else{
  ...
  return (
    <>
      <div className={`${classes.chart_out_per_label2} ${browserRedirect()!=1?classes.mobile_view:''} ${browserRedirect()===3&&classes.chart_out_per_label_tablet}`}>
      ...
      <div className={classes.chart_main_box_out} id={chatID}>
