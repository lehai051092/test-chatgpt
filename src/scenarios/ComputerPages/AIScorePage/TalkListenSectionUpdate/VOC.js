...
import { browserRedirect, getLocationState } from '../../../../utils/util';
...
const VOCSection = (props) => {
    ...
    const getVOCData = (url) => {
        const sessionState = getLocationState();

        let filterChatId = chatID?chatID:sessionState.chatId;
        try {
        if(filterChatId){
    ...
    return(
        ...
        <div className={`${classes.speak_analytic_reuslt} ${browserRedirect() === 3 ? classes.tablet_view_speak_analytic_reuslt : null} mb-3`}>
            {browserRedirect()!==3 || !isOverall ?
                <div className={classes.analytic_reuslt_header}>
                    <p idName={autoId()} className="font-18 mb-0 font-weight-bold ">発話速度</p>
                    <button className={`${classes.open_btn} ${classes.btn_width} ${ vVOCWrapperShowStatus[0] && classes.show}`} style={browserRedirect()===1&&isOverall?{height:'36px'}:null} onClick={() => closeVOCWrapper(0)}>
                        {browserRedirect()===1 && isOverall &&
                            <span className={`${classes.open_btn_text} ${ vVOCWrapperShowStatus[0] && classes.show}`} style={{padding:'0px 12px'}}>詳細の表示</span>
                        }
            ...
            <div className={classes.analytic_reuslt_body}>
                {browserRedirect()===3 && isOverall&&    
                    <div className={classes.analytic_reuslt_header}>
                        <p idName={autoId()} className="font-18 mb-0 font-weight-bold ">発話速度</p>
                        <button className={`${classes.open_btn} ${classes.over_all_btn_width} ${ vVOCWrapperShowStatus[0] && classes.show}`} onClick={() => closeVOCWrapper(0)}>
                            <span className={`${classes.open_btn_text} ${ vVOCWrapperShowStatus[0] && classes.show}`}>詳細の表示</span>
                            <span className={classes.open_btn_icon}></span>
                        </button>
                    </div>
                }
                <Row className={`mx-0 ${browserRedirect() === 3 ? `mb-0` : `mb-3`}`}>
                    <Col xs={browserRedirect()===3?`6`:`5`} className={`d-flex align-items-center px-0 ${isOverall?'justify-content-start pl-5':'justify-content-end'}`}>
                        {/** 文字/秒 */}
                        <h2 className={`font-16 font-weight-bold ml-0 ${isOverall && 'ml-lg-4 mb-3'} ${browserRedirect()===3&&classes.analytic_reuslt_body_detail}`}>{averageSpeed < 0 ? '全てテキスト' : averageSpeed + '文字/秒'}
                        </h2>
                    </Col>
                    <Col xs={browserRedirect()===3?`6`:`7`} className={classes.evaluation_btn_wrapper}>
                        <button className={`${classes.evaluation_btn} ml-auto`} >
                            <span>評価</span>
                            <span>
                                <img src={getSpeakingSpeedIcon(averageSpeed)} alt="cross_icon" class=""/>
                            </span>
                        </button>
                    </Col>
                </Row>
                {
                    ...
                    <>             
                    <div class={classes.evaluation_point}>
            ...
            {browserRedirect()!==3 || !isOverall ?
                <div className={classes.analytic_reuslt_header}>
                    <p idName={autoId()} className="font-18 mb-0 font-weight-bold">NGワード  
                        ...
                        <img onClick={() => closeVOCTextWrapper(0)} src={question_icon} alt="question_icon" class={`ml-3 cursor-pointer ${browserRedirect()===3?classes.analytic_reuslt_header_icon:null }`}/>
                        {
                    ...
                    <button className={`${classes.open_btn} ${classes.btn_width} ${ vVOCWrapperShowStatus[1] && classes.show}`} style={browserRedirect()===1&&isOverall?{height:'36px'}:null} onClick={() => closeVOCWrapper(1)}>
                        {browserRedirect()===1 && isOverall &&
                            <span className={`${classes.open_btn_text} ${ vVOCWrapperShowStatus[1] && classes.show}`} style={{padding:'0px 12px'}}>詳細の表示</span>
                        }
                        <span className={classes.open_btn_icon}></span>
                    </button>
                </div>:null
            ...
            <div className={classes.analytic_reuslt_body}>
                {browserRedirect()===3 && isOverall&&
                    <div className={classes.analytic_reuslt_header}>
                        ...
                        <button className={`${classes.open_btn} ${classes.over_all_btn_width} ${ vVOCWrapperShowStatus[1] && classes.show}`} onClick={() => closeVOCWrapper(1)}>
                            <span className={`${classes.open_btn_text} ${ vVOCWrapperShowStatus[1] && classes.show}`}>詳細の表示</span>
                            <span className={classes.open_btn_icon}></span>
                        </button>
                    </div>
                }
                <Row className="mb-3 mx-0">
                    <Col xs={browserRedirect()===3?`6`:`5`} className={`d-flex align-items-center px-0 ${isOverall?'justify-content-start pl-5':'justify-content-end'}`}>
                        <h2 className={`${classes.speaking_speed_font} mb-0 text-lg-left ml-0  ${isOverall && 'ml-lg-4'} ${browserRedirect()===3&&classes.analytic_reuslt_body_detail}`}>{ngWordsCount}回</h2>
                    </Col>
                    <Col xs={browserRedirect()===3?`6`:`7`} className={classes.evaluation_btn_wrapper}>
                        <button className={`${classes.evaluation_btn} ml-auto`} >
                            <span>評価</span>
                            <span>
                                <img src={getNgWordIcon(ngWordsCount)} alt="cross_icon" class=""/>
                            </span>
                        </button>
                    </Col>
                </Row>
                    ...
                    <Row className="mb-3 mx-0">
                    {
                            words.ng_words.map((item,index) => {
                                return(
                                    ...
                                    <div key={index}>
                                        {/* <CustomChip text={item.name} className={`${classes.free_story_custom_bg_chit} mb-1 ${findWordCount("NGワード",item.name)>0? classes.is_custom_bg:''}`}/> */}
        ...
        <div className={`${classes.speak_analytic_reuslt} ${browserRedirect() === 3 ? classes.tablet_view_speak_analytic_reuslt : null} mb-3`}>
            {browserRedirect()!==3 || !isOverall ?
                <div className={classes.analytic_reuslt_header}>
                    ...
                    <span className='d-inline-block position-relative'>
                        <img onClick={() => closeVOCTextWrapper(1)} src={question_icon} alt="question_icon" class={`ml-3 cursor-pointer ${browserRedirect()===3?classes.analytic_reuslt_header_icon:null }`}/>
                        {
                            vVOCWrapperShowTextStatus[1] && <span className={`${classes.expand_text_wrapper}`}>募集人が発した口ぐせの回数がカウントされます。</span>
                        }
                    </span>
                    </p>                
                    <button className={`${classes.open_btn} ${classes.btn_width} ${ vVOCWrapperShowStatus[2] && classes.show}`} style={browserRedirect()===1&&isOverall?{height:'36px'}:null} onClick={() => closeVOCWrapper(2)}>
                        {browserRedirect()===1 && isOverall &&
                            <span className={`${classes.open_btn_text} ${ vVOCWrapperShowStatus[2] && classes.show}`} style={{padding:'0px 12px'}}>詳細の表示</span>
                        }
                        <span className={classes.open_btn_icon}></span>
                    </button>
                </div>:null
            ...
            <div className={classes.analytic_reuslt_body}>
                {browserRedirect()===3 && isOverall&&
                    <div className={classes.analytic_reuslt_header}>
                        ...
                        <span className='d-inline-block position-relative'>
                            <img onClick={() => closeVOCTextWrapper(1)} src={question_icon} alt="question_icon" class={`ml-3 cursor-pointer ${browserRedirect()===3?classes.analytic_reuslt_header_icon:null }`}/>
                            {
                                vVOCWrapperShowTextStatus[1] && <span className={`${classes.expand_text_wrapper} ${classes.second_overall_expand_text_wrapper}`}>募集人が発した口ぐせの回数がカウントされます。</span>
                            }
                        </span>
                        </p>                
                        <button className={`${classes.open_btn} ${classes.over_all_btn_width} ${ vVOCWrapperShowStatus[2] && classes.show}`} onClick={() => closeVOCWrapper(2)}>
                            <span className={`${classes.open_btn_text} ${ vVOCWrapperShowStatus[2] && classes.show}`}>詳細の表示</span>
                            <span className={classes.open_btn_icon}></span>
                        </button>
                    </div>
                }
                <Row className="mb-3 mx-0">
                    <Col xs={browserRedirect()===3?`6`:`5`} className={`d-flex align-items-center px-0 ${isOverall?'justify-content-start pl-5':'justify-content-end'}`}>
                        <h2 className={`${classes.speaking_speed_font} mb-0 text-lg-left ml-0  ${isOverall && 'ml-lg-4'} ${browserRedirect()===3&&classes.analytic_reuslt_body_detail}`}>{habitCount}回</h2>
                    </Col>
                    <Col xs={browserRedirect()===3?`6`:`7`} className={classes.evaluation_btn_wrapper}>
                        <button className={`${classes.evaluation_btn} ml-auto`} >
                            <span>評価</span>
                            <span>
                                <img src={getHabitWordIcon(habitCount)} alt="cross_icon" class=""/>
                            </span>
                        </button>
                    </Col>
                </Row>
