...
const ChatLog = ({ id, messages, selectKeyword, MsgSelectScore,vAvatarName }) => {
    ...
    return (
        <div className={`${styles.main_div} ${browserRedirect()!=1 ? browserRedirect()===3 ? styles.tablet_view : styles.mobile_view : styles.pc_view}`} id="chat_list_container" name="chat_list_container">
                            ...
                            messages.map((msg, index) => {
                                if (msg.type == 'IncomingMessage') {
                                    return <div key={index} className={`${styles.msg} ${styles.left_msg}`} ref={vChatMsgRef.current[index]}>
                                        <div className={`${styles.msg_img} ${msg.text.length >20 ?styles.adjust_width:""} d-flex flex-wrap align-items-center`}>
