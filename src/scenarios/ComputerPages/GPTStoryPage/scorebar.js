...
const ScoreBar = ({ id, item = null}) => {
    ...
    <Row className={`${classes.scroll_bar} ${item.commitId != ""?'':classes.scroll_bar_hide_before} ${browserRedirect() === 3 ? classes.scroll_bar_tablet: null}`} id={autoId()} name={autoId()}>
            <Col xs="1.8">
                <span className={browserRedirect() === 3? classes.time_style_tablet: classes.time_style} id={`${id}_date_text`} name={`${id}_date_text`}>{date(item)}</span>
           </Col>
           <Col  xs="1.5" >
                <span className={browserRedirect() === 3 ? classes.sec_style: null} id={`${id}_time_diff`} name={`${id}_time_diff`} >{getTwoTime(item)}</span>
           </Col>
           <Col xs="0.5">
           ...
           <Col xs="1.5">
            ...
            {browserRedirect() === 3 ? <>
                    <Col xs="1.5" className={classes.correct_answer_text_tablet}>
                        <span className={`mr-1 pr-xl-2`} id={`${id}_correct_answer_text`} name={`${id}_correct_answer_text`}>{t('rateOfRisk.correct_answer_rate')}</span>
                        <span style={{fontSize:'22px'}} className=" mr-xl-6" id={`${id}_percent`} name={`${id}_percent`}>{percent(item)}</span>
                    ...
                    <Col xs="1.5" className={classes.just_center}>
                        <span style={{fontSize:'22px'}} className=" mr-xl-6" id={`${id}_percent`} name={`${id}_percent`}>{percent(item)}</span>
                        <span className={classes.scroll_bai}>%</span>
                    </Col>
                </>
           ...
           <Col xs="1.5" className={classes.last_col}>
                {
                    item.commitId ?
                    <span>
