...
import {browserRedirect, getLocationState, onDownload, setLocationState} from '../../../utils/util';
            ...
            return <div className={classes.foldableArea}>
              <div className={`${classes.purpose_sec} ${browserRedirect() === 3 ? classes.purpose_sec_tablet : null}`}>
                                <span id="rate_of_risk_header" name="rate_of_risk_header"
                                      style={browserRedirect() === 3 ? {height: '45px'} : {height: '79px'}}>
                                    <span
                                      className={browserRedirect() === 3 ? classes.firstTitle_tablet : classes.firstTitle}>{select_task.name}</span>
                                </span>
              </div>
              <div className={classes.historyList}>
                <div className={classes.top_style}>
                  ...
                  <Row
                    className={`mb-1 d-flex ${classes.no_wrap} ${browserRedirect() === 3 ? `${classes.fold_material_tablet} mt-4` : `mt-3`}`}>
                    <Col xs="1.5">
                      <div className={classes.adjust_div}>
                        <span className={classes.adjust_text_header2} id="header2"
                ...
                <div className="p-2 mt-4">
                  ...
                  <div
                    className={`mt-1 pb-2 cmn-scroll-bar ${classes.scroll_container} ${browserRedirect() === 3 ? classes.scroll_container_tablet : null}`}>
                    {
                      select_task.showStatus && vAiScore.get(select_task.taskID).map((item, index) => {
                        return <Link
                          to={{pathname: `/ai-score/${select_task.taskID}/${item.section.persona.id}`}}
                          key={index}
                          onClick={() => {
                            setLocationState({
                                item,
