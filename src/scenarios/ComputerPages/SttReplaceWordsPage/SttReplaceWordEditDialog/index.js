...
function SttReplaceWordEditDialog(props) {
  const {open, setOpen, title, item, handleRefresh} = props;
  const styles = useStyles();

  const [original, setOriginal] = useState('');
  const [replaced, setReplaced] = useState('');
  const [wordType, setWordType] = useState('INPROPRIATE_WORDS');
  ...
  const handleOK = async () => {
    if (original === '' || (wordType === 'HARD_REPLACE' && replaced === '')) {
    ...
    if (item && item.id) {
      ...
      const data = {
        ...
        replaceWord: wordType === 'HARD_REPLACE' ? replaced : original,
        userName: mstDBUserName ?? "",
      }
      updateReplaceWord(item.id, data).then((res) => {
        if (res.data) {
      ...
      <div className={styles.body}>
        ...
        <div className={styles.content}>
          <div className={styles.gridTitle}>
            <span>
              <span>必須</span>
              <span className={styles.infoTitle}>単語／語句</span>
            </span>
            <input
                type='text'
            ...
            {wordType === 'HARD_REPLACE' ? (
                <>
                  <span>
                    <span>必須</span>
                    <span className={styles.infoTitle}>置換後文字</span>
                  </span>
                  <input
                      type='text'
            ...
            <span>
              <span>必須</span>
              <span className={styles.infoTitle}>タイプ</span>
            </span>
            <div>
              <label>
                <input
                    type='radio'
                    name='wordType'
                    value={'INPROPRIATE_WORDS'}
                    defaultChecked={wordType === 'INPROPRIATE_WORDS'}
                    onChange={(e) => {
                      setWordType(e.target.value)
                    }}
                />
                <span style={{marginLeft: 5, fontWeight: 'normal'}}>音声認識精度</span>
              </label>
              <label>
                <input
                    type='radio'
                    name='wordType'
                    value={'HARD_REPLACE'}
                    defaultChecked={wordType === 'HARD_REPLACE'}
                    onChange={(e) => {
                      setWordType(e.target.value)
                    }}
                />
...
export default SttReplaceWordEditDialog;